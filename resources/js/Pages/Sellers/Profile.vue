    ...
    <app-layout>
        ...
        <div class="flex flex-row mt-5">
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                <button v-if="this.page == 'profile'" class="bg-green-600 hover:bg-green-800 text-white font-bold py-2 px-4 rounded-full" @click="page = 'profile'">
                    Mon Profil
                </button>
                <button v-else class="bg-green-400 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full" @click="page = 'profile'">
                    Mon Profil
                </button>
            </div>
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                <button  v-if="this.page == 'profile'" class="bg-green-400 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full" @click="page = 'card'" >
                    Ma Carte
                </button>
                <button  v-else class="bg-green-600 hover:bg-green-800 text-white font-bold py-2 px-4 rounded-full" @click="page = 'card'" >
                    Ma Carte
                </button>
            </div>
        </div>
        ...
        <form>
            <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="">
                    ...
                    <div class="mb-4">
                        <div class="grid grid-cols-8 gap-4">
                            <p class="col-span-7 block text-gray-700 font-bold mb-2">Téléphone : {{currentUser.phone_number}}
                            </p>
                        ...
                        <div v-if="hidden == currentUser.phone_number" class="grid grid-cols-6 gap-4">
                            ...
                            <button @click="userModify('phone_number',  tempContent), hidden = 'yes', tempContent = null"
                                class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-green-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-green-500 focus:outline-none focus:border-green-700 focus:shadow-outline-green transition ease-in-out duration-150 sm:text-sm sm:leading-5s"
                                type="button">Valider</button>
                            <button @click="hidden = 'yes', tempContent = null"
                                class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-red-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red transition ease-in-out duration-150 sm:text-sm sm:leading-5s"
    ...
    export default {
        ...
        methods: {
           ...
           async onSelected(event) {
                ...
                // await axios.get('/sanctum/csrf-cookie')
                var body = {profile_photo_path: this.profile_photo}
